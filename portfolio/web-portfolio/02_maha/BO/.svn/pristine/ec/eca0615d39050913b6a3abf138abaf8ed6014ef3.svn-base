...
// BO_TreatyMasterInfo input 자동 콤마
function comma(str) {
	str = String(str);
...
// BO_ContractAddMod_Popup 기타 클릭시 기타사항 입력란 활성화
$(document).ready(function () {

	// 라디오버튼 클릭시 이벤트 발생
	$("input:radio[name=dt_reason]").click(function () {

		if ($("input[name=dt_reason]:checked").val() == "1") {
			$("input:text[name=text]").attr("disabled", false);
			// radio 버튼의 value 값이 1이라면 활성화

		} else if ($("input[name=dt_reason]:checked").val() == "0") {
